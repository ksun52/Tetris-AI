from MCTS_heuristics import TetrisAI
...
for i in tqdm(range(num_iterations), desc="Optimizing", unit="iteration"):
    ...
    negative_heuristic_weights = np.random.uniform(-1, 0, size=4)
    positive_heuristic_weights = np.random.uniform(0, 1, size=1)
    heuristic_weights = np.concatenate((negative_heuristic_weights, positive_heuristic_weights), axis=0) 
