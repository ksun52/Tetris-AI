...
class TetrisMCTS:
    ...
    def __init__(self, max_simulations=SIM_COUNT, max_playout_depth=MAX_DEPTH, game=None, render=True):
    ...
    def _simulate(self, expanded_node):
        ...
        heuristic_value = (
            ...
            -0.1 * state_props[3] +  # Sum Height
            -0.3 * state_props[4]    # Max Height

        )

        heuristic_value += expanded_node.tetris_made * 5

        if self.mcts_game.game_over:
            heuristic_value -= 100
    ...
    def make_move(self, best_move):
        self.game.play(best_move.pos, best_move.rotation, render=self.render, render_delay=0.005)
...
class TetrisAI:
    def __init__(self, render=True):
        self.render=render
        self.game = Tetris()
        self.mcts = TetrisMCTS(max_simulations=SIM_COUNT, max_playout_depth=MAX_DEPTH, game=self.game, render=self.render)
